		...
		if addSoftDropSubjets:
			...
			_addProcessAndTask( proc, mod["packedPATJetsSoftDrop"],
				 cms.EDProducer("JetSubstructurePacker",
						jetSrc=cms.InputTag(mod["selPATJets"]), #changed from selPATJets
						distMax = cms.double( jetSize ),
						fixDaughters = cms.bool(False), #Changed to True to check a bug
						#packedPFCandidates = cms.InputTag("packedPFCandidates"), #Line Added
						algoTags = cms.VInputTag(
						cms.InputTag(mod["selPATJetsSoftDropPacked"])
						),
						algoLabels =cms.vstring('SoftDrop') #changed from Softdrop
						)
				 )
